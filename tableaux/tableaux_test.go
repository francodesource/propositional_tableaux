...
var bigFormula = formula.Parse("!!(((!!!(q ^ !!(!!(!!(!!(!z !| !z) !& !!(t !& (v !& !!z))) & !!!(!!v & !((!!z -> !(!!(!((x <-> x) ^ (!t <-> !v)) & p) !| (y -> q))) !& !!!!!!(t -> q)))) ^ !(!!!x -> v))) & ((r <-> (!!!(v !& (w & r)) ^ !!!(!z | !!(!s !| !p)))) !| (z -> p))) & !((!!!!((!!(s !| q) & !(!v !& !!v)) -> !(!!(!!!!(y !| !!!z) & !(!!t <-> !(t | v))) | !!!(!(!(s <-> t) !| !q) <-> !((!s & !p) -> q)))) -> (!y <-> !(!t & p))) <-> !(!p ^ !r))) & !((w -> !r) <-> !!!v))")
var hugeFormula = formula.Parse("!!(((!!!(!!!(!y | !(v !& !!q)) & !z) & !!z) !& (!(!((!!(!s <-> !r) <-> (!!v <-> !(s & !!!z))) !| !!(!x -> r)) -> !!((t <-> v) !| !!!(y -> q))) !| !u)) !| !!!(!((((!!!!!!((!!!x !& (y & x)) <-> v) -> !(u -> (v !& s))) | !!(!s !| p)) & (v !& (!p <-> !(!y ^ !p)))) & !!!((!!(!!!!(((((t ^ s) & (!p ^ !u)) | !(!!z -> !!(!!!(!!v <-> !p) & (w !& !t)))) <-> ((!!!!((!p !| !w) & !p) & (v !& ((y !& u) | !u))) -> !z)) -> (!!!!y ^ !v)) | (!!(v & !(v | t)) <-> ((t !& x) & v))) <-> !!(!(!!!!!(!(!y | t) !& w) & v) ^ !!(!(z !| w) !| (!!(!p & !p) | (v | !!z))))) <-> ((x ^ !w) ^ !!!((!(((y | r) -> !w) | (!w & !s)) | (!!(z ^ !z) -> !(x | w))) !& (!(!(!!x !| q) -> !w) !| !!p))))) & ((v <-> p) <-> v)))")
